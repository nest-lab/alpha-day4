...
JS_SNAKE.game = (function () {
  ...
  function init(frameLength, width=null, height=null) {
    var $canvas = $('#jsSnake');
    if ($canvas.length === 0) {
      $('body').append('<canvas id="jsSnake">');
    }
    $canvas = $('#jsSnake');
    $canvas.attr('width', width||JS_SNAKE.width);
    $canvas.attr('height', height||JS_SNAKE.height);
    canvas = $canvas[0];
    ctx = canvas.getContext('2d');
    score = 0;
    JS_SNAKE.frameLength = frameLength
  ...
  function gameLoop() {
    ...
    if (snake.checkCollision()) {
      ...
      timeout = setTimeout(gameLoop, JS_SNAKE.frameLength);
    }
  }
  ...
  function restart() {
    ...
    $(canvas).unbind('click');
    JS_SNAKE.game.init(JS_SNAKE.frameLength);
  }
  ...
  function bindEvents() {
    ...
    $(JS_SNAKE).bind('appleEaten', function (event, snakePositions) {
      ...
      score++;
      JS_SNAKE.frameLength *= 0.99; //subtle speed-up
    });
